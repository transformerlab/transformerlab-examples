...
# ============================================================================
# Monkey-patches for Transformers 5.x CSM compatibility
# Fix 1: In-place operation error in CsmDepthDecoderModel.forward()
#   The original code does `inputs_embeds[:, 0] = backbone_last_hidden_state`
#   which fails because inputs_embeds is a leaf tensor requiring grad.
#   We clone inputs_embeds before the in-place assignment.
# Fix 2: backbone_last_hidden_state rejected by _validate_model_kwargs in
#   depth_decoder.generate(). We override _validate_model_kwargs on the
#   depth decoder to accept this kwarg.
# ============================================================================
def _apply_csm_monkey_patches():
    """Apply monkey-patches to fix Transformers 5.x CSM compatibility issues."""
    try:
        from transformers.models.csm.modeling_csm import CsmDepthDecoderModel
        
        _original_depth_decoder_forward = CsmDepthDecoderModel.forward
        
        import functools
        @functools.wraps(_original_depth_decoder_forward)
        def _patched_depth_decoder_forward(self, input_ids=None, backbone_last_hidden_state=None,
                                           attention_mask=None, position_ids=None,
                                           past_key_values=None, inputs_embeds=None,
                                           use_cache=None, **kwargs):
            # Clone inputs_embeds before the in-place operation to avoid
            # "a view of a leaf Variable that requires grad is being used
            # in an in-place operation" error
            if inputs_embeds is not None:
                inputs_embeds = inputs_embeds.clone()
            elif input_ids is not None:
                # Let the original code create inputs_embeds from input_ids,
                # but we need to intercept after embed_tokens and before the
                # in-place assignment. We'll handle this by computing it ourselves.
                from transformers.utils.generic import is_torchdynamo_compiling
                ...
                position_ids_local = torch.arange(past_seen_tokens, past_seen_tokens + inputs_seq_length, device=device)
                
                codebook_idxs = torch.clamp(position_ids_local - 1, min=0)
                offset = codebook_idxs * self.vocab_size
                inputs_embeds = self.embed_tokens(input_ids + offset)
                
                # Clone to make it safe for in-place ops
                inputs_embeds = inputs_embeds.clone()
                
                input_ids_are_first_codebook = position_ids_local[0] == 0
                if backbone_last_hidden_state is not None:
                    ...
                    if not is_torchdynamo_compiling() and input_ids_are_first_codebook:
                        ...
                        logger.warning(
                            ...
                            "`backbone_last_hidden_state` should also be provided for correct inference."
                        )
                ...
                position_ids_local = position_ids_local.unsqueeze(0)
                position_embeddings = self.rotary_emb(hidden_states, position_ids=position_ids_local)
            ...
            return _original_depth_decoder_forward(
        ...
        CsmDepthDecoderModel.forward = _patched_depth_decoder_forward
        print("✅ Applied CSM monkey-patch: CsmDepthDecoderModel.forward (in-place fix)")
        
    except Exception as e:
        print(f"⚠️  Could not apply CsmDepthDecoderModel monkey-patch: {e}")
    ...
    try:
        ...
        def _patched_validate_model_kwargs(self, model_kwargs):
            """Remove backbone_last_hidden_state before validation since it's
            consumed by prepare_inputs_for_generation but rejected by the validator."""
            bhs = model_kwargs.pop("backbone_last_hidden_state", None)
            try:
                _original_validate(self, model_kwargs)
            except Exception:
                pass  # Don't let validation errors crash training
            if bhs is not None:
                model_kwargs["backbone_last_hidden_state"] = bhs
        
        CsmDepthDecoderForCausalLM._validate_model_kwargs = _patched_validate_model_kwargs
        print("✅ Applied CSM monkey-patch: CsmDepthDecoderForCausalLM._validate_model_kwargs (kwargs fix)")
        
    except Exception as e:
        print(f"⚠️  Could not apply CsmDepthDecoderForCausalLM monkey-patch: {e}")


# Apply patches immediately on import
_apply_csm_monkey_patches()
