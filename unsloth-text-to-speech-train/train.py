...
# Training configuration
    ...
    try:
        ...
        try:
            ...
            dataset = dataset.cast_column(training_config["_config"]["audio_column_name"], Audio(sampling_rate=training_config["_config"]["sampling_rate"]))
            max_audio_length = max(len(example[training_config["_config"]["audio_column_name"]]["array"]) for example in dataset)
        except Exception as e:
        ...
        try:
            ...
            if training_config["_config"]["model_architecture"] == "CsmForConditionalGeneration":
                ...
                lab.log(f"❌ Model architecture {training_config['_config']['model_architecture']} is not supported for audio training.")
